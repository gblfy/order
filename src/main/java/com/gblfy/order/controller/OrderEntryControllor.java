...
public class OrderEntryControllor {
    ...
    public String showMenus(Model model,
        ...
        if (useCookie != null && useCookie.equalsIgnoreCase("on")) {
            ...
            log.info("用户选择使用cookie，进入使用cookies的控制逻辑！");
            ...
            log.info("用户没有选择使用cookie，进入不使用cookies的控制逻辑！");
    ...
    public String saveMenu(Model model,
        ...
        if (mid >= 1) {
            log.info("保存菜品更新！");
            log.info("Request Param: mid = " + mid);
            log.info("Request Param: cid = " + cid);
            log.info("Request Param: mname = " + mname);
            log.info("Request Param: price = " + price);
